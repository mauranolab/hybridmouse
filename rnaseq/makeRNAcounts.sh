...
sample=`dirname $bam | xargs basename` # | cut -d '.' -f1
...
if [[ "$mappedgenome" == "C57B6" ]]; then
       ...
       refbam=`dirname $bam | perl -pe "s/$mappedgenome\$/C57B6\//g;"``basename $bam`
...
hotspotfile=/vol/isg/annotation/bed/mm10/gencodev10/GencodevM10.Exons.bed
...
/vol/mauranolab/mauram01/hybridmice/dnase/src/countReads.py - $refbam $bam --sample $sample --strain $strain --minReads 1 --require 3 --filter 2828 --minMAPQ 20 --permittedMismatches 2 --maxTemplateLength 9999999999999999 --minBaseQ 20 --cyclesToSkip 0 --maxReadLength 90 |
bedmap --delim '\t' --range 76 --echo --indicator - $indelfile | perl -pe's/1$/TRUE/;' -e's/0$/FALSE/;' |
closest-features --ec --delim '\t' --dist --closest - $indelfile | awk -F "\t" '{for(i=1; i<=NF-7; i++) {printf $i "\t"} printf $NF "\n"}' > $TMPDIR/$PREFIX.counts.txt


#used to be: mv $TMPDIR/$PREFIX.counts.txt $PREFIX.counts.txt
...
data <- read(filename)
colnames(data) <- c("chrom", "chromStart", "chromEnd", "rs", "totalReads", "ref.allele", "ref.reads", "nonref.allele", "nonref.reads", "totalReadsInclFailed", "skippedReads", "distToNearestSNP", "indel", "distToNearestIndel", "PercentGC")
...
data\$distToNearestSNP.bin <- cut(data\$distToNearestSNP, breaks=c(0, 76, 250, 500, 1000, Inf), right=F, include.lowest=T)
data\$distToNearestIndel.bin <- cut(abs(data\$distToNearestIndel), breaks=c(0, 76, 250, 500, 1000, Inf), right=F, include.lowest=T)
